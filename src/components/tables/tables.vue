<template>
  <div>
    ...
    >
      ...
      <!-- 自定义列插槽 -->
      <template v-for="colum in slotArr" :slot="colum.slot" slot-scope="{ row, index }">
        <slot :name="colum.slot" :row="row" :index="index" />
      </template>
      <!-- 操作列 -->
...
export default {
  ...
  methods: {
    ...
    handleColumns (columns) {
      this.slotArr = columns.filter(item => item.slot && item.slot !== 'action')
